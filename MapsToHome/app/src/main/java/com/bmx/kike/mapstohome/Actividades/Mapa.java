package com.bmx.kike.mapstohome.Actividades;
...
import com.bmx.kike.mapstohome.BuscarDireccion;
import com.bmx.kike.mapstohome.DirectionFinderListener;
import com.bmx.kike.mapstohome.Modelo.DaoRuta;
import com.bmx.kike.mapstohome.Pojos.Ruta;
import com.bmx.kike.mapstohome.R;
import com.bmx.kike.mapstohome.Route;
import com.bmx.kike.mapstohome.UbicacionActual;
...
public class Mapa extends FragmentActivity implements OnMapReadyCallback, DirectionFinderListener {
    ...
    private void trazarruta() {
        ...
        String destination = txtdestino.getText().toString().trim();
            ...
            if(txtorigen.getText().toString().trim().equalsIgnoreCase("MI UBICACION")){
                origin = ubicacion.getLatitude()+","+ubicacion.getLongitude();
            }else{
                origin = txtorigen.getText().toString().trim();
            }
        if(txtdestino.getText().toString().trim().equalsIgnoreCase("MI UBICACION")){
    ...
    private void agregarmarcador(LatLng position, String titulo, String info) {
    ...
    private void agregarmarcador1(LatLng position, String titulo, String info) {
        // Agregamos marcadores para indicar sitios de interéses.
        Marker mimarcador = mMap.addMarker(new MarkerOptions()
                .position(position)
                .title(titulo)  //Agrega un titulo al marcador
                .snippet(info)   //Agrega información detalle relacionada con el marcador
                .icon(BitmapDescriptorFactory.fromResource(R.mipmap.bici))); //Color del marcador
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_add:
                if(rutatrazada==1) {
                    DaoRuta dao = new DaoRuta(getApplication());
                    dao.Insert(new Ruta(txtorigen.getText().toString().trim(), txtdestino.getText().toString().trim()));
                    Toast.makeText(this, "Ruta Guardada", Toast.LENGTH_SHORT).show();
                    rutatrazada=0;
                }else{
                    Toast.makeText(this, "Debes Trazar la Ruta Para poder Guardar", Toast.LENGTH_SHORT).show();
                }
                return true;
            default:
